...
### ZisK's proving stack: STARK + SNARK

Two distinct proofs are produced at different stages, and any
discussion of "proof size" or "verify cost" needs to specify which one:

| Stage             | Proof system            | Underlying field | Size                  | Used for                                       |
| ----------------- | ----------------------- | ---------------- | --------------------: | ---------------------------------------------- |
| **Native prove**  | STARK (Plonky3 + PIL2)  | Goldilocks       | ~328 KB (any program) | Generated by `cargo-zisk prove`; off-chain verify in ~150 ms |
| **VADCOP wrap**   | STARK aggregation       | Goldilocks       | ~328 KB               | Recursive aggregation of multiple inner STARK proofs into one |
| **PLONK wrap**    | PLONK SNARK             | BN254            | 2,994 bytes total (768-byte proof + 256-byte publics + JSON vkey tail) | On-chain EVM verification via `PlonkVerifier.sol`; ~300–400 k gas |

The two-stage pipeline (STARK → SNARK wrap) is the same pattern RISC
Zero uses (wraps to Groth16) and SP1 uses (wraps to PLONK). It gets
you "STARK proving speed + SNARK verification cost" — generating a
PLONK SNARK directly from the original RISC-V execution trace would
be much slower than the STARK path; verifying a STARK directly on EVM
would be tens of millions of gas + 328 KB calldata.

**Measured on this PoC:**

| Step                                  | Wall-clock              | Output             |
| ------------------------------------- | ----------------------: | ------------------ |
| `cargo-zisk prove` (chain-replay)     | 228 s (CPU)             | 328 KB STARK proof |
| `cargo-zisk verify` (off-chain)       | 156 ms                  | bool               |
| `cargo-zisk wrap-proof --plonk`       | 139 s (CPU, peak ~60 GB) | 2,994-byte file with 768-byte EVM proof |
| `PlonkVerifier.verifySnarkProof()`    | typical ~300–400 k gas  | revert on bad proof |

So the security model is: **the STARK is what economically anchors the
computation** (Plonky3 + Goldilocks, post-quantum-secure assumptions);
the **SNARK wrap is a verifier-friendly compression** that adds the
trusted-setup assumption of PLONK over BN254. Any production
deployment on Ethereum-style EVM settles through the SNARK; off-chain
verifiers (light clients, indexers, bridges with their own verifier
binary) can use the STARK directly and keep the simpler crypto
assumptions.
